...
#     def __init__(self):
#         ## We don't currently handle special cases with
#         ## custom keys..
#         assert self._schema['keys'] == ['id']

#         orig_dict = {'id': None}
#         self._updates = {}

#         if len(self._schema['core']) > 0:
#             orig_dict['core'] = {}
#             self._updates['core'] = {}

#         for field in self._schema['special']:
#             if field == 'extras':
#                 orig_dict['extras'] = {}
#             elif field in ('groups', 'relationships', 'tags'):
#                 orig_dict[field] = []
...
#                 pass
#             else:
#                 raise ValueError("Unsupported special field: {0}"
#                                  .format(field))

#         self._original = orig_dict

#     @classmethod
#     def from_dict(cls, obj):
#         """Alternate constructor, pre-loading values from a dict"""

#         new_obj = cls()

#         ## Unfreeze it
#         orig_dict = dict(new_obj._original)
...
#                 orig_dict['core'][field] = value

#         for field in cls._schema['special']:
#             if field == 'resources':
#                 new_obj._resources = CkanDatasetResources(obj['resources'])

#             elif field in obj:
#                 ## Just copy them over, we'll handle them
#                 ## later, if needed..
#                 orig_dict[field] = obj[field]

#         new_obj._original = freeze(orig_dict)
#         return new_obj
...
#         if name in ('_original', '_resources'):
...
#         self._resources = []
#         if init is not None:
#             self._resources.extend(CkanResource.from_dict(r) for r in init)
...
#     def from_dict(cls, obj):
#         """
#         Instantiate a :py:class:`CkanOrganization`, preloading values from
#         a dictionary.
#         """
...
#         if self._updates['core'] != {}:
#             return True  # something has been updated
#         for field in ('groups', 'extras', 'tags', 'users'):
#             if (field in self._updates) and \
#                     (self._updates[field] != self._original[field]):
#                 return True
#         if self._resources.is_modified():
